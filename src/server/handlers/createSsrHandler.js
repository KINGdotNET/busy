...
export default function createSsrHandler(template) {
  return async function serverSideResponse(req, res) {
    try {
      ...
      return res.send(renderSsrPage(store, content, assets, template, req.hostname !== 'yehey-app.herokuapp.com'));
    } catch (err) {
      console.error('SSR error occured, falling back to bundled application instead', err);
      return res.send(renderSsrPage(null, null, assets, template, req.hostname !== 'yehey-app.herokuapp.com'));
    }
  };
}
